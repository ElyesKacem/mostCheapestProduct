...
entry = Entry(searchFrame,width=30)
entry.insert(0,"what you looking for",)
search = Label(searchFrame,text="Search: ")
searchButton = Button(searchFrame,text="Search",padx=10,pady=2,command = lambda: Search(entry.get()))
...
def Search(keyword):
    ...
    for item in data:
        ...
        raw_data = urllib.request.urlopen("https://openweathermap.org/themes/openweathermap/assets/img/logo_white_cropped.png").read()
        im = Image.open(io.BytesIO(raw_data))
        image3 = ImageTk.PhotoImage(im)
        logo2 = Label(itemFrame,image=image3)
